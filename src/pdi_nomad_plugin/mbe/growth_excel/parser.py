...
class ParserMbePDI(MatchingParser):
    def parse(self, mainfile: str, archive: EntryArchive, logger) -> None:
        ...
        for (
            ...
            # creating Substrate archives
            substrate_filename = (
                f'{substrate_id}_{substrate_index}.SubstratePDI.archive.{filetype}'
            )
            ...
            substrate_archive = EntryArchive(
                data=substrate_data if substrate_data else SubstrateMbe(),
        ...
        grown_sample_data = ThinFilmStackMbePDI(
        ...
        grown_sample_archive = EntryArchive(
            data=grown_sample_data if grown_sample_data else ThinFilmStackMbePDI(),
            m_context=archive.m_context,
            metadata=EntryMetadata(upload_id=archive.m_context.upload_id),
        )
        create_archive(
        ...
        samplecut_data = SampleCutPDI(
            name=str(sample_id) + ' sample cut',
            parent_sample=ThinFilmStackMbeReference(
        ...
        for _, row in samplecut_sheet.iterrows():
            ...
            children_sample_data = ThinFilmStackMbePDI(
        ...
        samplecut_archive = EntryArchive(
            data=samplecut_data if samplecut_data else SampleCutPDI(),
            m_context=archive.m_context,
            metadata=EntryMetadata(upload_id=archive.m_context.upload_id),
        )
        create_archive(
        ...
        for step_index, step in growthrun_sheet.iterrows():
            growth_step = GrowthStepMbePDI()
            growth_step.environment = ChamberEnvironmentMbe()
            ...
            growth_step.sample_parameters = [SampleParametersMbe()]
        ...
        growth_process_object = GrowthMbePDI()
        ...
        growth_process_filename = f'{sample_id}-growth.GrowthMbePDI.archive.{filetype}'
        # Activity.normalize(growth_process_object, archive, logger)
        growth_process_archive = EntryArchive(
            data=growth_process_object if growth_process_object else GrowthMbePDI(),
            m_context=archive.m_context,
            metadata=EntryMetadata(upload_id=archive.m_context.upload_id),
        )
        ...
        for step_id, step in pregrowth_sheet.iterrows():
            pregrowth_step = GrowthStepMbePDI()
            pregrowth_step.environment = ChamberEnvironmentMbe()
            ...
            pregrowth_step.sample_parameters = [SampleParametersMbe()]
        ...
        pregrowth_process_object = GrowthMbePDI()
        ...
        pregrowth_process_filename = (
            f'{sample_id}-pregrowth.GrowthMbePDI.archive.{filetype}'
        )
        ...
        experiment_filename = f'{sample_id}.ExperimentMbePDI.archive.{filetype}'
        experiment_data = ExperimentMbePDI(
            name='experiment',
            method='MBE 2 experiment',
            pregrowth=GrowthMbePDIReference(
